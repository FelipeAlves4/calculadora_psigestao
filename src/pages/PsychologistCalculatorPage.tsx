  ...
  const comparison = <>
    <section className="content-section">
      <div className="section-heading">
        ...
        <p className="mt-2 max-w-4xl text-sm leading-relaxed text-slate-300">{projectionSummary(current, projected)}</p>
      </div>
      <PsychologistComparisonCards current={current} projected={projected} />
      <div className="mt-4"><PsychologistComparisonTable rows={rows} /></div>
    </section>
    <PsychologistResultsCharts current={current} projected={projected} />
  </>;

  return <div className={presentationMode ? 'presentation-mode min-h-screen' : 'min-h-screen'}>
    <Header
      title="Calculadora de Resultados para Psicólogos"
      subtitle="Compare seu cenário atual com uma projeção de atendimentos, faturamento e impacto profissional."
      brandName="PsiGestão"
      brandNote="Uma solução ASEX Educação"
      userLabel="Usuário conectado"
      ...
      onLogout={logout}
    />
    <PsychologistPrintableReport current={current} projected={projected} rows={rows} />
    <main className="mx-auto grid w-full max-w-7xl gap-4 px-3 py-4 sm:gap-6 sm:px-6 sm:py-6 lg:px-8">
      <div className="print-report-heading" aria-hidden="true">
        <div><strong>Calculadora PsiGestão</strong><span>Cenário atual e projeção de atendimentos</span></div>
      ...
      <nav className="mobile-tabs" aria-label="Seções da calculadora">
        {([['current', 'Atual'], ['projected', 'Projetado'], ['comparison', 'Comparação']] as const).map(([key, label]) => <button key={key} type="button" className={tab === key ? 'mobile-tab mobile-tab-active' : 'mobile-tab'} onClick={() => setTab(key)}>{label}</button>)}
      </nav>
      <div className="scenario-grid">
        <div className={tab === 'current' ? 'mobile-panel-active' : 'mobile-panel-hidden'}><PsychologistScenarioCalculator title="Cenário Atual" scenario={state.current} results={current} onChange={change('current')} accent="current" /></div>
        <div className={tab === 'projected' ? 'mobile-panel-active' : 'mobile-panel-hidden'}><PsychologistScenarioCalculator title="Cenário Projetado" scenario={state.projected} results={projected} onChange={change('projected')} accent="projected" /></div>
      </div>
      <div className={tab === 'comparison' ? 'mobile-panel-active comparison-stack' : 'mobile-panel-hidden comparison-stack'}>{comparison}</div>
    </main>
    {confirmingClear ? <div className="modal-backdrop" role="presentation" onMouseDown={(event) => event.target === event.currentTarget && setConfirmingClear(false)}>
      <div className="confirm-modal" role="alertdialog" aria-modal="true" aria-labelledby="clear-title">
        ...
        <div className="confirm-actions">
          ...
          <button type="button" className="confirm-danger" onClick={() => { setState(emptyState()); setConfirmingClear(false); }}>Limpar valores</button>
        </div>
      </div>
    </div> : null}
  </div>;
